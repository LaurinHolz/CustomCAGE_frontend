...
import { buildNameTranslation } from "../utils/translateNames";

export default function OverviewPanel({ state }) {
  const { hostNames } = buildNameTranslation(state);
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
        <thead>
          <tr style={{ borderBottom: '0.5px solid var(--color-border-tertiary)' }}>
            {["Host","Backend name","Type","Zone","Services","Decoys","Attack paths"].map(h => (
        ...
        <tbody>
          {state.hosts.map(h => {
            ...
            const backendName = hostNames[h.name] ?? "—";
            return (
              <tr key={h.id} style={{ borderBottom: '0.5px solid var(--color-border-tertiary)' }}>
                <td style={{ padding: '6px 8px', fontWeight: 500 }}>{h.name}</td>
                <td style={{ padding: '6px 8px', color: 'var(--color-text-tertiary)', fontFamily: 'monospace' }}>{backendName}</td>
                <td style={{ padding: '6px 8px' }}><span style={{ ...S.badge, fontSize: 9 }}>{h.type}</span></td>
                <td style={{ padding: '6px 8px', color: zone?.color || 'var(--color-text-secondary)' }}>
                  {zone ? <>{zone.name} <span style={{ fontFamily: 'monospace', fontSize: 9, opacity: 0.6 }}>({zone.id})</span></> : "—"}
                </td>
